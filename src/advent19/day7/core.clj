...
; 54321
;(def input "3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0")
; 65210
;(def input "3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,\n1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0")
...
(defn output [computer modes]
  (let [val (read-with-mode computer 1 (mode modes 0))]
    (inc-pc (assoc computer :output val) 2)))
...
(defn execute-amplifiers [phase-settings]
  (loop [phases phase-settings
         prev-output 0]
    (if-let [phase (peek phases)]
      (let [computer {:pc 0 :program program :input prev-output :phase phase}
            output (:output (execute-computer computer))]
        (recur (pop phases) output))
      prev-output)))
